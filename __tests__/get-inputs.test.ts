...
describe('showInputs()', () => {
  ...
  test('print all inputs GithubToken', () => {
    ...
    process.env['INPUT_GITEE_TOKEN'] = 'test_gitee_token';

    const inps: Inputs = getInputs();
    showInputs(inps);

    const authMethod = 'GiteeToken';
    const test = getInputsLog(authMethod, inps);
    assertWriteCalls([`${test}${os.EOL}`]);
  });
  ...
  test('print all inputs PersonalToken', () => {
    delete process.env['INPUT_DEPLOY_KEY'];
    delete process.env['INPUT_GITEE_TOKEN'];
...
describe('getInputs()', () => {
  test('get default inputs', () => {
    ...
    expect(inps.GiteeToken).toMatch('');
  ...
  test('get spec inputs', () => {
    ...
    expect(inps.GiteeToken).toMatch('test_gitee_token');
